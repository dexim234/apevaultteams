...
export const DayStatusForm = ({ type, status, onClose, onSave }: DayStatusFormProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-xl flex items-start sm:items-center justify-center z-[70] p-4 sm:p-6 touch-manipulation overflow-y-auto overscroll-contain modal-scroll">
      <div className={`w-full max-w-4xl rounded-3xl shadow-[0_24px_80px_rgba(0,0,0,0.45)] border ${theme === 'dark' ? 'bg-gradient-to-br from-[#0c1320] via-[#0b1220] to-[#08111b] border-white/10' : 'bg-gradient-to-br from-white via-slate-50 to-white border-slate-200'} max-h-[85dvh] sm:max-h-[calc(100dvh-96px)] flex flex-col`}>
        <div className="flex-shrink-0 p-4 sm:p-6 lg:p-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              onClick={onClose}
              className={`p-2.5 rounded-full border flex-shrink-0 ${theme === 'dark' ? 'border-white/10 text-gray-200 hover:bg-white/5' : 'border-slate-200 text-slate-600 hover:bg-slate-100'}`}
              aria-label="Закрыть"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain modal-scroll px-4 sm:px-6 lg:px-7">
